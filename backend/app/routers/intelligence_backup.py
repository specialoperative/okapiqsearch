...
async def comprehensive_market_scan(request: MarketScanRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        # 1. Google SERP aggregation - super fast with aggressive timeout
        for query in search_queries[:2]:  # Only 2 queries for maximum speed
            try:
                # Use asyncio timeout for hard timeout
                import asyncio
            ...
            priority=1
                ...
                crawl_res = await asyncio.wait_for(crawler_hub._execute_crawl(crawl_req), timeout=5.0)
                ...
                logger.warning(f"SERP search timed out for '{query}' after 5 seconds")
            except Exception as e:
                logger.warning(f"SERP search failed for '{query}': {e}")
        
        # 2. Google Maps (Apify) aggregation if insufficient results - temporarily disabled for speed
        if False and len(all_businesses) < 10:
            try:
        ...
        # 3. Yelp aggregation for additional data - temporarily disabled for speed
        if False and len(all_businesses) < 5:
            try:
